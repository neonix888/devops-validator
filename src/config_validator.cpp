...
ValidationResult ConfigValidator::validateDirectory(const std::string& dirPath) {
    ...
    try {
        for (const auto& entry : fs::recursive_directory_iterator(dirPath)) {
            if (entry.is_regular_file()) {
                ...
                if (ext == ".json" || ext == ".yaml" || ext == ".yml" ||
                    ext == ".toml" || ext == ".env" || path.find(".env") != std::string::npos) {

                    std::cout << "\n" << Color::BOLD << "Validating: " << path << Color::RESET << std::endl;
        ...
        overallResult.errors.push_back(std::string("Directory scan error: ") + e.what());
        overallResult.valid = false;
    }

    std::cout << "\n" << Color::BOLD << "=== Directory Validation Summary ===" << Color::RESET << std::endl;
...
ValidationResult ConfigValidator::validateJSON(const std::string& content, const std::string& filePath) {
    ValidationResult result;
    ...
    try {
        ...
        result.valid = false;
        result.errors.push_back("JSON parse error at byte " + std::to_string(e.byte) + ": " + e.what());
    }

    return result;
}

ValidationResult ConfigValidator::validateYAML(const std::string& content, const std::string& filePath) {
    ValidationResult result;
...
ValidationResult ConfigValidator::validateTOML(const std::string& content, const std::string& filePath) {
    ValidationResult result;
...
ValidationResult ConfigValidator::validateEnv(const std::string& content, const std::string& filePath) {
    ValidationResult result;
    result.fileType = "ENV";
    ...
    result.valid = true;
    Utils::printInfo("Found " + std::to_string(validVars) + " environment variables");

    return result;
}

void ConfigValidator::printValidationResult(const ValidationResult& result, const std::string& filePath) {
    if (result.valid) {
    ...
    for (const auto& error : result.errors) {
        std::cerr << Color::RED << "  ERROR: " << error << Color::RESET << std::endl;
    }

    for (const auto& warning : result.warnings) {
        std::cout << Color::YELLOW << "  WARNING: " << warning << Color::RESET << std::endl;
    }
}
